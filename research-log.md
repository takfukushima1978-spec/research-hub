...
#### TBP 昇格候補
- **TBP-003候補**（2026-06-22 提案・確認待ち **59日目**）:「着手前に実態（git）と文書（backlog）の一致を確認する」— Tak の確認待ち継続。⚠️ **2ヶ月超経過。要アクション判断。**
- **TBP-004候補**（2026-06-22 提案・確認待ち **59日目**）:「不可逆性で安全方向を決めるが、カテゴリ丸ごとの保守化は目的を殺す」— Tak の確認待ち継続。⚠️ **2ヶ月超経過。要アクション判断。**

#### 再検討トリガー該当
- **TBP-001 再評価トリガー（マネーフォワード AI Cowork が Claude Agent SDK + MCP を採用・2026年7月）**: マネーフォワード AI Cowork がバックオフィス業務の自律処理基盤として Claude Agent SDK と MCP を公式採用。TBP-001「外部ツール導入は審査→最小権限→段階拡張」が直接適用対象の外部 AI エージェントサービスとして正式登録候補。書き込み系（仕訳登録・経費精算登録）は全 deny からスタートを推奨（freee-mcp と同様）。
- **TBP-001 再評価トリガー（v2.1.237 Concise 出力スタイル追加）**: 本日リリースの v2.1.237 で「Concise」出力スタイルが新設。結論先出し・前置き・ナレーション省略モードで、Routines のデバッグ時の可読性設計に影響する可能性。TBP-001「外部ツール利用時の出力形式設計」への参考追記候補。

---

### 外部リサーチ（機能B）
#### 参照した情報源
- Claude Code 公式チェンジログ（⭐⭐⭐⭐⭐）: v2.1.237（2026-08-20 本日リリース）・v2.1.236（2026-08-19）を確認
- Anthropic 公式ブログ（⭐⭐⭐⭐⭐）: Amazon 5GW コンピュート拡張、$30B ランレート突破、Claude Sonnet 5 価格永続化
- anthropics/claude-code GitHub issues（⭐⭐⭐⭐⭐）: 8/20 新着 #88385〜#88391 を確認
- Qiita/note（⭐⭐⭐）: Claude Code 8月前半アップデートまとめ記事、Claude Code 週次まとめ（8/2週）確認
- 会計×AI: freee 固定資産管理リリース・マネーフォワード AI Cowork・kaikei-ai.jp 2026年8月版確認

#### 🔴 即座に適用すべき事項

**1. 🔴 Claude Code v2.1.237（2026-08-20 本日リリース）— アップデート推奨**

本日最新リリース。主要変更:

- **「Concise」出力スタイル追加（/config で選択可能）**: Claude が「結論先出し・前置き・ナレーションなし・実作業は従来と同等の徹底度」で応答する新出力スタイル。Routines でのデバッグ時に出力量を削減しつつ品質を維持したい場合に有効
- **プロンプトキャッシュ修正（LLM ゲートウェイ・カスタム base URL 使用セッション対象）**: LLM ゲートウェイまたはカスタム base URL を使用するセッションでプロンプトキャッシュが正常に機能しないバグを修正。Research Hub Routines が Anthropic API を直叩きする場合への影響は低いが、Worker/Proxy 経由のセッションには影響あり

- 🔴 **アクション**: `npm update -g @anthropic-ai/claude-code` で v2.1.237 にアップデート

**2. 🔴 Claude Code v2.1.236（2026-08-19 リリース）— 前日リリース確認**

前日（8/19）のリリース。主要変更:

- **`ANTHROPIC_DEFAULT_MODEL` 環境変数追加**: 新規セッションの開始モデルを環境変数で固定できる（`/model` コマンドによる上書きは引き続き有効で、再起動後も維持）。`ANTHROPIC_MODEL`（既存）との違いは「デフォルト設定」vs「固定設定」。Routines 環境変数でモデルを明示したい場合に有用
- **`notify_when_idle` for cross-session SendMessage**: 同一マシン上の別 Claude Code セッションが次にアイドル状態になった際に1回だけ通知を受け取る opt-in 機能（macOS/Linux）。Routines 間の連携設計に応用可能

- 🔴 **アクション**: `ANTHROPIC_DEFAULT_MODEL` を Routines 環境変数に設定する場合は v2.1.236 以降に更新が必要

#### 🟡 近いうちに試したいこと（上位3件）

**1. 🟡 Concise 出力スタイルの Routines デバッグ活用（v2.1.237）**

- Routines のデバッグ時に出力が長大になりがちな auto-research-collect や deep-research-runner で Concise スタイルを試すことで、セッションログの可読性が向上する可能性
- `/config` で Output Style → Concise を選択するだけで有効化可能
- 🟡 **アクション**: 次回の手動デバッグセッションで Concise モードを有効にして使用感を確認。特に deep-research-runner の長文出力改善への影響を評価

**2. 🟡 `ANTHROPIC_DEFAULT_MODEL` で Routines モデルを環境変数化（v2.1.236）**

- 従来は Routines プロンプト内でモデル指定が必要だったが、環境変数で一元管理できるようになった
- `ANTHROPIC_DEFAULT_MODEL=claude-opus-5` と設定することで、全 Routines を一括で Opus 5 に切り替えることも可能
- 🟡 **アクション**: 「Cloudflare Workers_My Research」環境設定の環境変数セクションに `ANTHROPIC_DEFAULT_MODEL=claude-sonnet-5` の追加を検討（コスト vs 品質のトレードオフを確認してから）

**3. 🟡 freee 固定資産管理（2026年5月〜）+ マネーフォワード AI Cowork の実務評価**

- freee: 固定資産の登録・減価償却・除却・売却・部門按分を freee 会計・freee 申告と連携して一元管理する「freee固定資産」が 2026年5月から順次提供開始。経理部長として固定資産管理フローの効率化に直結
- マネーフォワード AI Cowork: 2026年7月リリース済み。Claude Agent SDK + MCP で仕訳・経費精算・請求書処理を自然言語で自律実行。全プランで MCP サーバー提供中
- 🟡 **アクション**: freee 固定資産の利用可能プランと移行タイミングの確認。マネーフォワード AI Cowork の trial 申し込みと TBP-001 に従った 4 軸審査を計画

#### 🟢 参考情報

- **Anthropic × Amazon 5GW コンピュート拡張（2026-08月）**: Amazon との協定で最大 5 ギガワットの新規コンピュート容量（Trainium2 含む）を確保。ランレート収益は $300億超（$9B/2025年末から急拡大）。API・Routines の長期安定供給の根拠としてさらに強化
- **GitHub Issues 8/20 新着（#88385〜#88391）**: データロスバグ（#88388、tools 絡み）・セキュリティ問題（#88387、重複扱い）・macOS デスクトップリグレッション（#88390）・Windows モデルバグ（#88389）・機能要望（CLI 系、#88391/#88385/#88386）が本日新規オープン。headless/Routines への直接影響は確認されず
- **Anthropic Claude Sonnet 5 価格永続化（2026-08-10 再確認）**: $2/$10/Mtok が永久標準価格。昨日レポートで既報。Routines のコスト計画は現行価格で永続して計算可能
- **会計×AI トレンド継続（2026年8月）**: freee・マネーフォワード・バクラクの3社が「AIエージェント型 SaaS 標準搭載」フェーズ。AI 導入企業の経費精算工数 75-90% 削減事例が報告水準として定着。freee Agent Hub の正式ローンチ（8/28）まで **残り8日**。申告書チェックエージェント（無償）の体験準備を整えておくこと

#### references.md 更新提案

**新規追加提案（2026-08-20）**:

19. **v2.1.237 Concise 出力スタイル（/config 選択）**: 結論先出し・ナレーション省略の新スタイル。Routines デバッグ時の出力設計に影響する可能性。参照: `/config` → Output Style → Concise
20. **v2.1.236 `ANTHROPIC_DEFAULT_MODEL` 環境変数**: 新規セッション開始モデルを環境変数で設定可能に（`ANTHROPIC_MODEL` との違いは「デフォルト」vs「固定」）。Routines 環境変数での一元管理に有用
21. **マネーフォワード AI Cowork（2026-07 GA）**: Claude Agent SDK + MCP 基盤の国産バックオフィス AI エージェント。TBP-001 適用対象として審査推奨。書き込み系は全 deny からスタート
...
2026-08-21（明日）
注目点:
① **freee Agent Hub 正式ローンチ（8/28）残り8日**: 最終事前情報収集・申告書チェックエージェント無償体験の申し込み準備
② **Claude Code 新リリース確認**: v2.1.238 以降
③ **TBP-003・TBP-004 候補（60日目）**: 提案から丁度2ヶ月。Tak への確認アクション最優先
④ **マネーフォワード AI Cowork の TBP-001 審査計画**: TBP-001 に従った 4 軸チェック実施日程を確定
⑤ **Anthropic テキスト透かし検出 API**: 近日公開予定。公開次第 Research Hub 記事への影響評価
...
#### TBP 昇格候補
- **TBP-003候補**（2026-06-22 提案・確認待ち **57日目**）:「着手前に実態（git）と文書（backlog）の一致を確認する」— Tak の確認待ち継続。2ヶ月近く経過（要アクション判断）。
- **TBP-004候補**（2026-06-22 提案・確認待ち **57日目**）:「不可逆性で安全方向を決めるが、カテゴリ丸ごとの保守化は目的を殺す」— Tak の確認待ち継続。2ヶ月近く経過（要アクション判断）。

#### 再検討トリガー該当
- **TBP-001 再評価トリガー（Anthropic Risk Report August 2026 公開）**: 2026-08-14 公開のリスクレポートで、重大なミスアライメントリスクが「very low」→「**low**」に格上げ。理由は特定の失敗ではなく「全体的な不確実性の増大」。同レポートで未公開の内部モデル「**Model 2**」を開示（Mythos 5 を上回る能力だが安全評価未完了で外部非公開）。さらに、2025年5月〜2026年4月の133百万件のヒューマンフィードバックベンダートラフィックが生物兵器分類器なしで処理されていた事実も開示。TBP-001「外部ツール安全性評価」の「AI提供会社の安全報告書モニタリング」セクション追記候補。
- **TBP-001 再評価トリガー（Anthropic テキスト透かし実装・2026-08-02 施行）**: EU AI Act 第50条対応として Anthropic が全 Claude テキスト出力に不可視の透かし（SynthID-Text 方式）を全世界で実装開始。2026-08-02 以降に発行されたモデルは即座に対応、既存モデルも順次対応中。検出 API は近日公開予定。Research Hub の auto-research-collect・deep-research-runner が生成する記事本文にも透かしが埋め込まれている可能性がある。TBP-001「外部ツール出力の真正性評価」に「AI生成コンテンツの透かし存在確認」を追記提案。

---

### 外部リサーチ（機能B）
#### 参照した情報源
- Claude Code 公式チェンジログ（⭐⭐⭐⭐⭐）: v2.1.235（2026-08-18 本日リリース）を確認
- Anthropic 公式ブログ（⭐⭐⭐⭐⭐）: Anthropic Risk Report August 2026（8/14）・テキスト透かし実装（8/2〜）確認
- anthropics/claude-code GitHub（⭐⭐⭐⭐⭐）: 8/18 リリース確認
- clauding.de / havoptic.com / claudeupdates.dev: v2.1.234・v2.1.235 詳細確認
- 会計×AI: freee Agent Hub（8/28 ローンチ）・マネーフォワード AI Cowork・ai-revolution.co.jp 2026年8月版確認

#### 🔴 即座に適用すべき事項

**1. 🔴 Claude Code v2.1.235（2026-08-18 本日リリース）— アップデート推奨**

本日リリースの最新バージョン:

**新機能**:
- **スペルチェック設定（オプション）**: プロンプト入力欄でのリアルタイムスペルチェック。`aspell`・`hunspell`・`ispell` のいずれかがインストール済みの場合、入力中の誤字に下線が表示される。日本語入力への影響は限定的（英字入力にのみ作用する見込み）
- マークダウンリスト描画の修正
- プロンプト入力ハイライトの修正
- パーミッションプロンプト修正
- エージェントツールエラーメッセージの改善

- 🔴 **アクション**: `npm update -g @anthropic-ai/claude-code` で v2.1.235 にアップデート

**2. 🔴 Anthropic Risk Report August 2026（2026-08-14）— 安全設計見直しのトリガー**

Anthropic が2回目の会社全体リスクレポートを 2026-08-14 に公開。主要ポイント:

- **ミスアライメントリスク格上げ（very low → low）**: 具体的な失敗事例ではなく「2月以降の不確実性増大」が理由。特にベンチマークの飽和（benchmark saturation）が評価精度を下げている
- **Model 2 の開示**: Mythos 5 を能力で上回る内部モデルが存在するが、事前安全評価が未完了のため外部公開予定なし。能力推定の信頼度が低い段階であることを Anthropic 自身が認めている
- **生物兵器分類器なしのベンダートラフィック**: 2025年5月〜2026年4月の133百万件の人間フィードバックベンダー交換（約50,000名のコントラクター）が生物兵器分類器なしで処理されていた事実を開示
- **非新規兵器支援リスク**: 「low, but higher than our previous estimate」に引き上げ
- 🔴 **アクション**: Research Hub で Claude を利用した記事生成の品質管理フローに「AI提供会社の安全報告書」モニタリングを組み込む設計検討を推奨。直接の Routines 設定変更は不要だが、今後の Claude モデル安全性評価の参考にする。

**3. 🔴 Anthropic テキスト透かし実装（2026-08-02 施行）— Research Hub 記事への影響確認要**

EU AI Act 第50条対応。全世界で Claude テキスト出力に不可視の透かしを実装:

- **実装方式**: Google DeepMind の SynthID-Text をベースとした手法。生成時に確率的に文字列を選択し、統計的に検出可能な透かしを埋め込む。可読性・品質・速度・価格には影響なし
- **対象**: 2026-08-02 以降に公開されたモデルは即対応。既存モデルも順次対応中
- **検出 API**: 近日公開予定。将来的に Research Hub の記事コンテンツを外部が「AI生成」と識別できるようになる
- Research Hub の auto-research-collect・deep-research-runner・feedback-article-runner が生成する記事本文には現時点で透かしが埋め込まれている可能性がある
- 🔴 **アクション**: Anthropic の検出 API が公開されたらテスト実施を検討。研究記事の「AI生成」明示が将来の要件になる可能性に備え、記事スキーマに `is_ai_generated: true` フラグを追加することを中期計画として記録

#### 🟡 近いうちに試したいこと（上位3件）

**1. freee Agent Hub 正式ローンチ（2026-08-28）— 残り10日**

- 8/28（freee Advisor Day 2026 福岡）で「freee顧問先管理 | AIエージェント」と「freee Agent Hub」を同時正式公開
- 申告書チェックエージェント（無償）・月次チェックエージェント（期間限定無償→9/15有償）・記帳・ルール整備エージェントが提供開始
- freee Agent Hub: 士業事務所独自のカスタムエージェント構築・共有基盤。認定アドバイザー向け API も提供
- 🟡 **アクション**: 8/28 当日に申告書チェックエージェントの無償体験を試す。顧問税理士への事前確認。Research Hub でリリース当日の記事化を事前スケジューリング

**2. v2.1.235 スペルチェック機能の Routines 影響確認**

- 日本語入力主体の Routines では影響は限定的とみられるが、英語のプロンプトや出力指示内の英単語チェックが有効になる可能性
- 🟡 **アクション**: 次回セッション起動時にスペルチェックの有効化を確認。aspell/hunspell がインストール済みか確認し、不要なら無効化設定を検討

**3. Anthropic Decart 買収（$6B）続報確認**

- 前回報告（8/15〜8/17）から公式発表なし。交渉継続中とみられる
- 成立すれば Claude の推論速度・コスト効率が大幅改善 → deep-research-runner の応答コスト削減の可能性
- 🟡 **アクション**: Anthropic の公式発表を継続監視。現時点では Routine 設計変更は不要

#### 🟢 参考情報

**Anthropic Q2 2026 収益（2026-08-14 報告）**
- 第2四半期収益: **$11.5B**（前四半期比で急拡大）
- 2025年末の $9B ランレートから継続成長。Research Hub の観点: Anthropic の API・Routines の長期安定供給の根拠がさらに強化

**freee Agent Hub（8/28 ローンチ）詳細**
- オープンβ（5月〜）→ 正式提供（8/28〜）への移行
- β版フィードバックから「共通定型業務の自動化」と「事務所独自ルールへのカスタマイズ」の2軸が確認され、2サービスへの分離展開が決定
- 9/3（大阪）・9/15（東京）でも体験セッション実施予定

**会計×AI 一般（2026年8月時点）**
- freee・マネーフォワード・バクラクの3社が「AIエージェント標準搭載」フェーズへ移行済み（各社AIエージェント機能を本体機能として内蔵）
- バクラク AI エージェント: 契約管理 AI エージェントが夏提供予定のまま継続監視（正式リリース日未確定）
- 2026年の会計 AI 主要事例: 経費精算工数 75% 削減・請求書処理 80% 削減・月次締め 2 営業日前倒し

**Anthropic テキスト透かし技術詳細**
- 実装: 生成時（generation-time）に確率的に語彙選択を調整 → 統計的に「Anthropic のモデルが生成した」と識別可能なパターンを埋め込む
- EU の罰則: 違反時に最大 €1,500万または全世界年商の3%（高い方）。Anthropic は全世界適用で対応
- 古いモデルへの後付け対応: 「coming months」に実施予定

#### references.md 更新提案

前回（8/16・8/17）の未更新リスト（1〜15）に以下を追加:

**新規追加提案（2026-08-18）**:
16. **Anthropic Risk Report August 2026（8/14 公開）**: ミスアライメントリスク「very low」→「low」格上げ・Model 2 開示・生物兵器分類器欠落事案の開示。AI安全性評価フレームワークの継続的モニタリング根拠として記録推奨
17. **テキスト透かし実装（2026-08-02 施行）**: Research Hub 記事への影響確認要。記事スキーマへの `is_ai_generated` フラグ追加を中期計画として記録提案
18. **v2.1.235 スペルチェック機能（opt-in）**: Routines への直接影響は低いが設定確認推奨
※ 直接更新は行わない。Tak の確認後に実施。

#### 新規発見ソース候補

- **anthropic.com/aug-2026-risk-report**: Anthropic 公式リスクレポート（Aug 2026）。安全性モニタリングの一次情報として最優先（評価候補: ⭐⭐⭐⭐⭐）
- **support.claude.com/en/articles/16266773**: Claude AI 生成コンテンツのマーク方法（公式ヘルプ）。透かし実装の詳細確認先（評価候補: ⭐⭐⭐⭐⭐）

#### 次回リサーチ推奨日

2026-08-19（翌日）
注目点:
① **freee Agent Hub ローンチ（8/28）残り9日**: 事前情報・機能詳細の追加収集
② **Claude Code 新リリース確認**: v2.1.236 以降
③ **Anthropic Decart 買収交渉の続報**: $6B 交渉継続中（8/17 時点）。公式発表待ち
④ **TBP-003・TBP-004 候補（58日目）**: 継続確認リマインド
⑤ **Anthropic テキスト透かし検出 API 公開**: 近日公開予定。公開次第 Research Hub への影響評価
⑥ **Anthropic Risk Report Model 2 詳細**: 安全評価プロセスの完了タイミング・公開可能性の続報
...
#### TBP 昇格候補
- **TBP-003候補**（2026-06-22 提案・確認待ち **56日目**）:「着手前に実態（git）と文書（backlog）の一致を確認する」— Tak の確認待ち継続。2ヶ月近く経過（要アクション判断）。
- **TBP-004候補**（2026-06-22 提案・確認待ち **56日目**）:「不可逆性で安全方向を決めるが、カテゴリ丸ごとの保守化は目的を殺す」— Tak の確認待ち継続。2ヶ月近く経過（要アクション判断）。

#### 再検討トリガー該当
- **TBP-001 再評価トリガー（v2.1.234 Windows NT-namespace パス拒否）**: 本日リリースの v2.1.234 で、remote file reads・session restore・CLAUDE.md includes・workflow scripts・file uploads が Windows NT-namespace パス（`\\?\` 形式）を拒否するセキュリティ強化。TBP-001「外部ツール導入審査」のファイルアクセス制御評価項目への追記候補。
- **TBP-001 再評価トリガー（Anthropic サイバーセキュリティ評価・実世界事例）**: Anthropic が「Investigating three real-world incidents in our cybersecurity evaluations」を公開。実世界サイバーセキュリティ事例をモデル評価に反映した研究。TBP-001「外部ツール安全性評価」の参考資料として記録推奨。

---

### 外部リサーチ（機能B）
#### 参照した情報源
- Claude Code 公式チェンジログ（⭐⭐⭐⭐⭐）: v2.1.234（2026-08-17 本日リリース）を確認
- Anthropic 公式ブログ（⭐⭐⭐⭐⭐）: 2026-08-17 時点最新記事確認
- anthropics/claude-code GitHub issues（⭐⭐⭐⭐⭐）: 8/17 新着確認（awesome-claude-code 中心）
- freee prtimes（corp.freee.co.jp）: Agent Hub 8/28 ローンチ詳細確認
- goodway.co.jp: マネーフォワード 連結決算レビューエージェント提供開始確認
- 会計×AI一般: 2026年最新動向確認

#### 🔴 即座に適用すべき事項

**1. 🔴 Claude Code v2.1.234（2026-08-17 本日リリース）— アップデート推奨**

本日リリースの最新バージョン。前バージョン（v2.1.233、8/14）から以下が追加:

**新機能**:
- **`CLAUDE_CODE_PROJECT_DIR_NAME` 環境変数**: セッション固有 config ディレクトリを持つホスト向け。per-project トランスクリプトディレクトリの短縮名を指定可能
- **`selection:clear` キーバインドアクション**: テキスト選択のクリア。agents ビューでも動作
- **GitLab MR バッジ（footer・statusline）**: GitLab リモートと認証済み glab CLI がある場合、MR !N（draft/pending/green 状態付き）をフッターとステータスラインに表示
- **利用制限リセット時の自動セッション継続**: claude.ai の利用制限がリセットされると自動でセッション継続（/config でオフ可能）

**セキュリティ・プライバシー強化**:
- **Windows NT-namespace パス拒否**: remote file reads・session restore・CLAUDE.md includes・workflow scripts・file uploads が `\\?\` パスを拒否。事前承認前のファイルアクセスを強化
- **アカウントメール識別のみ使用**: Claude はアカウントメールを「ユーザー識別のみ」に使用し、無関係サービスへの送信を禁止（Tak の ask なしでは送らない）

- 🔴 **アクション**: `npm update -g @anthropic-ai/claude-code` で v2.1.234 にアップデート

**2. 🔴 Anthropic 公式ブログ 新着（2026-08-17 時点）**

- **「Improving Fable 5 Safeguards」**: Fable 5 モデルの生物学的セーフガード改善に関する記事公開。安全性設計に影響
- **「Investigating three real-world incidents in our cybersecurity evaluations」**: 実世界サイバーセキュリティ事例を Claude の評価ベンチマークに活用した研究。モデルの安全性評価フレームワークへの反映が示唆
- **「Expanding our use of Google Cloud TPUs and Services」**: Google Cloud TPU・サービス利用拡大を発表。推論インフラ強化 → 将来的な Claude 応答速度改善の布石
- **マルチエージェントシステムのパターンと課題（2026-08-13）**: 新興マルチエージェントシステムのパターンと問題点に関する研究発表。Research Hub のアーキテクチャ設計に参考
- **リーマン仮説関連研究（2026-08-10）**: 未公開研究版 Claude がリーマン仮説の問題でゼロ点の下限を 41.6% → 67.2% に改善。純粋数学への応用可能性を示す

#### 🟡 近いうちに試したいこと（上位3件）

**1. freee Agent Hub 正式ローンチ（2026-08-28）— 残り11日**

8/28（freee Advisor Day 2026、福岡）で以下が同時提供開始:

- **freee 顧問先管理 | AIエージェント**（税理士事務所向け）:
  - ①記帳エージェント ②ルール整備エージェント ③月次チェックエージェント ④申告書チェックエージェント の4種
  - 申告書チェックエージェント: 8/28 より**無償提供**。freee 申告の法人税申告書類 + BS・PL を横断チェックし申告前の論点を提案
  - 月次チェックエージェント: 期間限定無償 → 9/15 より有償
- **freee Agent Hub**（顧問ビジネス特化 AIエージェント開発基盤）:
  - 事務所独自ニーズへのカスタムエージェント作成・事務所内共有が可能
  - 認定アドバイザー向け API も提供

- 🟡 **アクション**: 8/28 当日に申告書チェックエージェントの無償体験申し込み。顧問税理士への事前共有・連携方法確認。Research Hub でリリース当日の記事化を事前スケジューリング

**2. マネーフォワード 連結決算レビューエージェント — 詳細確認継続**

- 「マネーフォワード クラウド連結会計」に「連結決算レビューエージェント」提供開始を goodway.co.jp で確認
- 背景: Money Forward AI Vision 2026（2026-04）発表の「AI Cowork」戦略の一環。会計・税務・財務・労務・法務の各バックオフィスを AI アシスタントが横断支援
- 経理部長（Tak 本業）に直結する高付加価値機能
- 🟡 **アクション**: マネーフォワード公式サイトで連結決算レビューエージェントの機能詳細・料金体系を確認。自社連結決算フロー（または担当範囲）での導入可能性評価

**3. v2.1.234 Windows NT-namespace パス拒否の Routines 影響確認**

- 新しいセキュリティ強化で `\\?\` 形式のパスが CLAUDE.md includes・workflow scripts 等で拒否される
- Research Hub の Routines は Linux sandbox で動作のため直接影響は低いが、将来的に Windows 環境からテスト・デバッグする際のパス形式に注意が必要
- 🟡 **アクション**: 次回 Routines 設定変更時に NT-namespace パスが混入していないか確認

#### 🟢 参考情報

**Claude Code GitHub Issues（2026-08-17 新着）**
- awesome-claude-code リポジトリ: #2551〜#2555 がリソース登録・バリデーション系 issues（コア機能への直接影響なし）
- claudish（多言語版 Claude Code）: #191・#192 がメンテナンス系
- コア機能（anthropics/claude-code 本体）に対する重大 bug 報告は 8/17 時点で確認されず

**会計×AI 一般トレンド（2026-08 時点）**
- 日本企業の経理部門 AI 導入率: **24.3%**（2026年4月時点）。AI 活用者の 75.6% が作業効率向上を実感
- 2026年の主要トレンド: **「ツール操作型 SaaS」→「AIエージェント型 SaaS」への進化**。従来の1タスク補助から複数業務ステップの自律実行へ
- 経費精算の自動化: 入力・確認工数 75% 削減の事例が多数。請求書処理は入力工数 80% 削減の可能性
- 実装原則: **「人が判断、AIが作業」**。仕訳候補・要約・ドラフト生成は AI、最終承認・例外処理は人間

**Anthropic 人事**
- **Mariano-Florentino（Tino）Cuéllar 氏**が 2026-08-04 に Anthropic の **Chief Global Affairs Officer** として入社。グローバル政策・規制対応強化

**Auto モード Routines 観察継続（前回からの継続）**
- 8/14 以降、全新規セッションで auto モードがデフォルト。classifier によるツール許可判断は無料
- 8/17 時点で Routines の auto mode 下での動作安定性は観察継続中

#### references.md 更新提案

前回（8/16）の未更新リスト（1〜11）に以下を追加:

**新規追加提案（2026-08-17）**:
12. **v2.1.234 `CLAUDE_CODE_PROJECT_DIR_NAME` 環境変数**: per-project トランスクリプトディレクトリ名の短縮設定。Routines 設計に関連
13. **v2.1.234 Windows NT-namespace パス拒否**: 事前承認前のファイルアクセスのセキュリティ強化。TBP-001 の「ファイルアクセス制御」評価項目への追記候補
14. **v2.1.234 自動セッション継続（利用制限リセット時）**: Routines の長時間セッションで制限ヒット後の自動復帰。設計考慮事項として記録推奨
15. **v2.1.234 メール識別のみ使用明示**: Claude のシステムプロンプトでアカウントメールの用途制限を明記。プライバシー設計の参考
※ 直接更新は行わない。Tak の確認後に実施。

#### 新規発見ソース候補

- **goodway.co.jp（グッドウェイ Fintech）**: マネーフォワード 連結決算レビューエージェントの詳細確認に使用。フィンテック情報ポータルとして会計×AI 追跡に有用（評価候補: ⭐⭐⭐）
- **gradually.ai/en/changelogs/claude-code**: Claude Code チェンジログの月別まとめサイト。8月分をまとめて確認可能（評価候補: ⭐⭐⭐）

#### 次回リサーチ推奨日

2026-08-18（翌日）
注目点:
① **freee Agent Hub ローンチ（8/28）残り10日**: 事前情報・機能詳細の追加収集
② **Claude Code 新リリース確認**: v2.1.235 以降（週明け 8/18 以降も続く可能性）
③ **マネーフォワード 連結決算レビューエージェント 詳細確認**: 機能・料金ページの直接確認
④ **Anthropic Decart 買収交渉の続報**: $6B 交渉継続中（8/15 時点）。公式発表待ち
⑤ **TBP-003・TBP-004 候補（57日目）**: 56日経過。Tak への確認リマインド継続
⑥ **Fable 5 セーフガード改善の詳細確認**: サイバーセキュリティ評価研究と合わせて TBP-001 への影響評価
...
#### TBP 昇格候補
- **TBP-003候補**（2026-06-22 提案・確認待ち**55日目**）:「着手前に実態（git）と文書（backlog）の一致を確認する」— Takの確認待ち継続。1ヶ月超経過（要アクション判断）。
- **TBP-004候補**（2026-06-22 提案・確認待ち**55日目**）:「不可逆性で安全方向を決めるが、カテゴリ丸ごとの保守化は目的を殺す」— Takの確認待ち継続。1ヶ月超経過（要アクション判断）。

#### 再検討トリガー該当
- **TBP-001 再評価トリガー（EU AI Act テキスト透かし義務化、2026-08-02 施行）**: EU AI Act 第50条による AI 生成テキストへの機械可読透かし義務が 2026-08-02 に施行。将来の Claude モデルが生成テキストに透かしを埋め込む方向に。TBP-001「外部ツール導入審査」の4軸チェック（出力の信頼性評価）に「AI生成透かし」の検証フロー追加を検討推奨。サブエージェント出力の事実確認設計にも影響。

---

### 外部リサーチ（機能B）
#### 参照した情報源
- Claude Code 公式チェンジログ（⭐⭐⭐⭐⭐）: v2.1.233（8/14）以降の更新確認
- Anthropic 公式ブログ（⭐⭐⭐⭐⭐）: 2026-08-16 時点最新記事確認
- anthropics/claude-code GitHub issues（⭐⭐⭐⭐⭐）: 8/16 新規 issues（#87204〜#87209）確認
- Zenn / Qiita: 2026-08 Claude Code 記事確認
- 会計×AI: freee Agent Hub / マネーフォワード 連結決算レビューエージェント 最新状況

#### 🔴 即座に適用すべき事項

**1. 🔴 Auto モード初回 Routines 実行確認（最優先）**
- 2026-08-14 より Pro/Max/Team 全新規セッションで auto モードがデフォルト化。classifier によるツール許可自動判断のオーバーヘッドが無料化。
- 初回の Routines 実行（8/14-8/15 夜: auto-research-collect 3:03 JST、auto-claude-code-watch 4:00 JST）が auto モードで動作したはず。
- 🔴 アクション: Console の Routines 実行ログを確認し、auto モードへの切替後も正常動作しているか検証。classifier の動作（判断速度・ツール許可パターンの変化）を確認。自動判断で予期せず省略されたツール呼び出しがないか重点チェック。

**2. マネーフォワード 連結決算レビューエージェント（2026-08-07 リリース）— 本日初確認**
- マネーフォワードが 2026-08-07 に「連結決算レビューエージェント」をリリース。連結会計データの異常値・整合性リスクを AI が自動分析するサービス。
- 経理部長・組織内会計士の業務に直結する高付加価値 AI 機能。
- 🔴 アクション: マネーフォワード Business サイトで詳細確認。自社の連結決算フロー（または担当範囲）での導入可能性・料金体系を評価。

**3. Claude Code v2.1.233（2026-08-14）以降 — 新規リリース未確認**
- 8/16 時点で v2.1.234 以降の公式リリースは確認できず。v2.1.233 が現時点での最新版。
- v2.1.233 の主要内容: auto モードの全 Pro/Max/Team プランデフォルト化・classifier オーバーヘッド無料化。
- 次期リリースは週明け（8/18 月曜）以降が想定される。

#### 🟡 近いうちに試したいこと（上位3件）

**1. freee Agent Hub 正式ローンチ（2026-08-28）— 残り12日**
- freee が 2026-08-28 に「Agent Hub」を正式ローンチ予定。申告書チェックエージェント（無料）が第一弾。
- 経理業務の AI エージェント活用の具体的な起点。Tak の本業（経理部長）に直結。
- 🟡 アクション: 8/28 リリース当日に申告書チェックエージェントを試す準備（freee アカウントの権限確認）。Research Hub でリリース当日の記事化を事前スケジューリング。

**2. EU AI Act テキスト透かしの Claude 実装影響調査**
- EU AI Act Article 50（透かし義務化）が 2026-08-02 施行済み。AI 生成テキストへの機械可読透かし埋め込みが欧州向けに義務化。
- Research Hub の auto-research-collect・deep-research-runner が生成する記事コンテンツへの影響（透かし埋め込み有無）を調査推奨。
- 🟡 アクション: Anthropic の公式対応スケジュール（どのモデルから・いつから透かし実装予定か）を追跡。

**3. Claude Code auto モードの Routines クラシファイアー挙動観察**
- 8/14 から auto モードがデフォルトになり、classifier がツール許可の自動判断を担う。
- Routines での自動判断パターン（何が省略され、何が許可されるか）の実データ収集が急務。
- 🟡 アクション: auto-research-collect・auto-claude-code-watch の次回実行結果を詳細にログ確認。classifier が想定通りのツール判断をしているかを評価し、必要なら設定調整を検討。

#### 🟢 参考情報

**GitHub Issues 新着（2026-08-16）**
- #87204〜#87209 が本日新規オープン（6件）。
- 内容: Desktop バグ・Windows パス処理・TUI 改善・plugins 関連。
- headless モード・Routines 動作に直接影響する bugs は確認されず。

**Anthropic 公式ブログ（2026-08-16 時点最新）**
- **EU AI Act テキスト透かし（2026-08-02 施行）**: Article 50 対応として AI 生成コンテンツへの機械可読透かし埋め込みが欧州向けに義務化。将来の Claude モデルが対応予定。
- **希少疾患研究助成金（AI for Science）**: 2026-07-20 発表の継続。Rare Disease Research Grants の公募が進行中。

**auto モード（2026-08-14 全展開）詳細**
- Pro/Max/Team 全プランで新規セッション開始時に auto モードがデフォルト選択。
- classifier によるツール許可の自動判断オーバーヘッドが無料化（追加課金なし）。
- 既存の継続セッションはリセットまで従来動作を維持。新規セッション開始時から auto モード適用。
- `claude auto-mode reset` コマンドで手動リセット可能（v2.1.210 以降）。

**会計×AI（2026-08-16 時点）**
- **マネーフォワード 連結決算レビューエージェント**（2026-08-07 リリース・本日初確認）: 連結会計データの異常値・整合性リスクを AI が自動分析。経理部長・CFO 向け高付加価値機能。詳細確認要。
- **freee Agent Hub**（2026-08-28 ローンチ予定・残り12日）: 申告書チェックエージェント（無料）を皮切りに会計×エージェント機能を展開予定。
- **バクラク 契約管理 AI エージェント**: 夏提供予定のまま継続監視（正式リリース日未確定）。

**Zenn / Qiita（2026-08-16 時点）**
- auto モードのデフォルト化に関する解説記事が複数公開。「auto モードで Routines はどう変わるか」の実験報告が増加中。
- Claude Code 学習マップ関連（Research Hub スタンプラリー）の実践記事も増加傾向。

#### references.md 更新提案

継続未確認項目（前回 8/15 から継続）:
1. **EndConversation ツール**（v2.1.214）: ジェイルブレイク対応ツールとして追記検討
2. **Bash 権限チェック厳格化**（v2.1.214）: 10,000文字超・zsh変数修飾子・help/man が確認対象
...
8. **MCP ツール出力メモリリーク修正**（v2.1.217）
9. **Agentic Misalignment 公式研究（Summer 2026）**: TBP-001 審査設計への追記提案

**新規追加提案（2026-08-16）**:
10. **EU AI Act テキスト透かし義務化（2026-08-02 施行）**: TBP-001「外部ツール導入審査」4軸チェックに「AI生成透かしの検証フロー」追加提案。将来の Claude モデルが透かしを埋め込むため、サブエージェント出力の真正性確認手法にも影響する可能性。
11. **auto モードデフォルト化（2026-08-14）**: Pro/Max/Team 全プランで classifier が無料オーバーヘッドで自動ツール判断。Routines 設計で auto モードの挙動観察を推奨事項として記録提案。
※ 直接更新は行わない。Tak の確認後に実施。

#### 新規発見ソース候補

- **claudeissues.com**: Claude Code GitHub Issues を整理した外部サービス。#87204-#87209 の確認に活用（評価候補: ⭐⭐⭐）
- **releasebot.io/updates/anthropic/claude-code**: Claude Code リリース通知サービス。v2.1.233 の詳細確認に使用（評価候補: ⭐⭐⭐）

#### 次回リサーチ推奨日

2026-08-17（翌日）
注目点:
① **Auto モード Routines 実行確認**: 8/15-8/16 夜（3:03・4:00 JST）実行ログを 8/17 午前に確認
② **freee Agent Hub ローンチ（8/28）残り11日**: 準備状況・事前情報収集継続
③ **マネーフォワード 連結決算レビューエージェント 詳細確認**: 製品ページ・機能・料金体系
④ **Claude Code 新リリース確認**: v2.1.234 以降（週明け 8/18 以降に予想）
⑤ **TBP-003・TBP-004 候補（56日目）**: 55日経過継続。Tak への確認リマインド継続
...
#### TBP 昇格候補
- **TBP-003候補**（2026-06-22 提案・確認待ち **51日目**）:「着手前に実態（git）と文書（backlog）の一致を確認する」— Tak の確認待ち継続。⚠️ **51日経過。Tak への最優先アクション要請。**
- **TBP-004候補**（2026-06-22 提案・確認待ち **51日目**）:「不可逆性で安全方向を決めるが、カテゴリ丸ごとの保守化は目的を殺す」— Tak の確認待ち継続。⚠️ **51日経過。Tak への最優先アクション要請。**

#### 再検討トリガー該当
- **TBP-001 再評価トリガー（auto mode 8/14デフォルト化・2日前）**: 8/14（明後日）から Pro/Max/Team の全新規セッションで auto mode がデフォルトになる。前回レポートからの継続。Research Hub Routines（headless セッション）への影響を最終確認すること。
- **TBP-001 再評価トリガー（Sonnet 5 価格永久化）**: Anthropic が Sonnet 5 の価格（$2/$10/Mtok）を 2026-08-10 に永久価格として確定。9/1 からの値上げ（$3/$15）は撤回。TBP-001「段階拡張時のコスト計算」の前提として、Routines に Sonnet 5 を使う場合のコスト試算は「永久に現行価格」として計算可能になった。プロモーション終了カウントダウンは不要になったため、従来の「8月中にコスト試算を完了」アクションの緊急度が低下。
...
#### 参照した情報源
- Claude Code 公式チェンジログ（⭐⭐⭐⭐⭐）: v2.1.228（2026-08-11）が最新。前回レポート（8/11）より1リリース進む
- Anthropic 公式ブログ（⭐⭐⭐⭐⭐）: Sonnet 5 価格永久化（8/10確定）、Fable 5 バイオセーフガード改善、Google・Broadcom 次世代コンピュートパートナーシップ拡大を確認
- anthropics/claude-code GitHub issues（⭐⭐⭐⭐⭐）: 8/12 新着 #86204〜#86210 を確認
- Qiita（⭐⭐⭐）: Claude Code 8月アップデート「エージェント組織の統治」視点記事（berrylove氏）、コマンド完全ガイド【2026年8月版】（superdora-cloud氏）確認
- 会計×AI: luvina.jp・TOKIUM・ai-market.jp・ai-revolution.co.jp・MoneyForward 2026年8月版確認

#### 🔴 即座に適用すべき事項

**1. 【最重要】Sonnet 5 価格永久化（$2/$10/Mtok）— 計画前提の更新が必要**

Anthropic が 2026-08-10 に確定発表:
- **変更前の想定**: Sonnet 5 のプロモーション価格（$2/$10/Mtok）は 2026-09-01 に通常価格（$3/$15/Mtok）へ値上がり予定だった
- **変更後**: $2/$10/Mtok が永久標準価格に決定。9/1 以降も値上がりなし
- **Routines への影響**: 「8月中にコスト試算を完了させる」アクションの緊急度が消滅。Sonnet 5 を Routines に適用する場合、現行価格のまま長期計画に組み込んで良い
- 🔴 **アクション**: scheduled-tasks.md のコスト計算欄・プロンプトにプロモーション価格終了の前提が含まれている場合は除去。Sonnet 5 は永久に $2/$10/Mtok として計画する

**2. Claude Code v2.1.228（2026-08-11）— self-hosted-runner の安定性修正**

前回レポート（8/11, v2.1.227）より1リリース進んだ。主な修正点:
- インタラクティブセッションがレイアウトエラー後に描画停止したままプロセスは継続するバグを修正
- Windows で Claude Code を git インストールフォルダの親フォルダから起動した場合に git/Git Bash が見つからないバグを修正
- `/tui` が `/model` 変更後に古いモデルに巻き戻すバグを修正
- cross-session messaging がインストール/アップグレード後の初回セッションで inbox なしに起動するバグを修正
- Remote Control `/resume` が接続中セッションに再開先の会話タイトル/履歴をリークするバグを修正
- **self-hosted-runner: checkout hook がプッシュしないリポジトリで失敗してもセッションが落ちなくなった**
- **self-hosted-runner: バックグラウンドタスク完了後とフォローアップターン開始の間のギャップでセッションが終了するバグを修正**
- 🔴 **アクション**: Claude Code を v2.1.228 にアップデート。self-hosted-runner を評価中の場合は修正済みバージョンでテスト。

**3. 【継続】Auto mode 8/14デフォルト化まで2日（本日 8/12）**

- Pro/Max/Team プランは 8月14日から auto mode がデフォルト
- Routines（headless セッション）への影響を最終確認する段階
- 🔴 **アクション**: scheduled-tasks.md と各 Routine の permission 設定を今日中に確認し、8/14 以降の初回 Routine 実行（3:03 JST）での挙動変化に備える

#### 🟡 近いうちに試したいこと（上位3件）

**1. self-hosted-runner の本格評価（v2.1.228 安定化を受けて）**

- v2.1.224（8/7）追加の `claude self-hosted-runner` が v2.1.228 で安定性修正を受けた
- Routines の Cloudflare bot 検知問題の根本解決策として引き続き最有力
- 🟡 アクション: Team/Enterprise プランであれば、ローカル環境での PoC を計画。Supabase 直叩き問題の代替手段として実測評価を推奨

**2. claude agents の「信頼ディレクトリ確認」挙動の把握（v2.1.221 変更）**

Qiita 記事（berrylove氏, 8/12付け）で指摘:
- claude agents が **信頼していないディレクトリ**に対してはワークスペースの信頼確認を挟むようになった
- Routines で `claude agents` を活用している場合、ディレクトリ信頼設定を確認
- 🟡 アクション: Research Hub の Routine 実行ディレクトリが信頼済みとして登録されているか確認。未確認の場合、8/14 auto mode 移行後に確認ダイアログで止まる可能性あり

**3. freee Agent Hub 正式提供（8/28）— 本業直結（継続）**

- 8/28 の freee Advisor Day 2026（福岡）で正式ローンチ予定
- 士業向け AI エージェント基盤（記帳・ルール整備・月次チェック・申告書チェックの4プリセット）
- 🟡 アクション: 顧問税理士・社内税務担当への影響確認を 8/28 前に完了させる

#### 🟢 参考情報

- **GitHub Issues 8/12 新着（#86204〜#86210）**: macOS 関連バグ多数（メモリ機能バグ #86210、デスクトップ/パーミッション系 #86207、モデル関連 #86206）、Windows agent-view リグレッション #86209、機能リクエスト（キーバインド #86205、statusline #86204）。headless/Routines への直接影響は確認されず
- **Anthropic の収益急成長（2026年）**: ランレート $300億超（2025年末 $90億から急拡大）、$100万超の企業顧客が1000社超（2月比で2ヶ月以内に倍増）。Anthropic の事業基盤強化は API・Routines の継続安定供給の根拠になる
- **Google・Broadcom との次世代コンピュートパートナーシップ拡大（8/12）**: 複数ギガワットの次世代コンピュートを調達する契約。Claude のインフラ基盤が大幅強化される見通し
- **Fable 5 バイオセーフガード改善（8/12）**: バイオセキュリティ評価における安全性をさらに強化。Fable 5 の Routines 利用が将来的に解禁される場合の安全性根拠として記録
- **Anthropic 数学的能力の進歩**: 未公開研究版 Claude がリーマン予想に関連する問題で長年の下限（41.6%）を 67.2% まで改善。研究用途での Claude 活用可能性が広がる
- **会計×AI（2026年8月）**: AIエージェント型 SaaS への移行加速が継続。経理AI導入率約24%、導入企業の68.3%が業務時間短縮を実感。月次締め2営業日前倒し事例が増加。Tak の本業（経理部長・組織内会計士）において AI エージェント型 SaaS の評価・導入判断が本格的に求められるフェーズ継続
- **Claude Sonnet 4.6（2/17リリース、参考）**: Sonnet 5 に次ぐ現役モデル。コーディング・コンピュータ利用・長期推論に強み。1M コンテキスト（β）。Routines での選択肢として記録

#### references.md 更新提案
**提案あり（継続）**:
1. **Sonnet 5 価格永久化の反映**: references.md にモデル価格を記載している場合、「プロモーション価格」ではなく「標準価格 $2/$10/Mtok（2026-08-10 永久化確定）」への更新を提案
2. **auto mode デフォルト化（8/14〜）**: 前回からの継続提案。パーミッションモード記述を更新。Tak 確認待ち

※ 実際の更新は Tak 確認後に実施。

#### 新規発見ソース候補
- **releasebot.io/updates/anthropic/claude-code**: Anthropic リリースノートを集約。8/11 に Claude Code 専用ページが確認できた。前回（8/9）提案済みのため継続候補として記録
- なし（新規発見ソースなし）
...
② Routines（3:03 JST, 4:00 JST）の auto mode 後初回実行ログ確認
③ TBP-003・TBP-004候補の確認依頼（51日経過・要アクション）
④ v2.1.229 以降のリリース確認
...
#### TBP 昇格候補
- **TBP-003候補**（2026-06-22 提案・確認待ち44日目）:「着手前に実態（git）と文書（backlog）の一致を確認する」— Takの確認待ち
- **TBP-004候補**（2026-06-22 提案・確認待ち44日目）:「不可逆性で安全方向を決めるが、カテゴリ丸ごとの保守化は目的を殺す」— Takの確認待ち

#### 再検討トリガー該当
- **TBP-001 再評価トリガー（マルチモデル障害 2026-08-05）**: 本日、Claude Mythos 5・Fable 5・Opus 5・Sonnet 5 の複数モデルで同時にエラー増加が観測された。Research Hub の Routine が auto モードで複数モデルを切り替えながら実行される場合、障害時のフォールバック設計が TBP-001「外部ツール導入審査」の可用性評価項目として直接関連する。「複数モデルが同時に障害を起こすシナリオ」をリトライ・フォールバック設計に組み込む必要があることを追記する材料として記録。
- **TBP-001 再評価トリガー（v2.1.222 セキュリティ修正）**: worktree 分離セッションが main ブランチに対して破壊的 git コマンドを実行できた脆弱性と、PreToolUse フックの auto-allow バイパス問題が修正。TBP-001「外部ツール最小権限・段階拡張」の worktree 設計レビューおよびフック設定の実効性確認を追記提案。
...
**1. マルチモデル同時障害（2026-08-05 観測）— Claude Mythos 5・Fable 5・Opus 5・Sonnet 5**
- **概要**: 本日（8/5）、Claude Mythos 5・Fable 5・Opus 5・Sonnet 5 の 4 モデルで同時にエラー増加が報告された。自動モードで動作する Routine が複数モデルを使用している場合、今回のような広範障害でフォールバックが機能しない可能性がある。
- **Research Hub への影響**: auto-research-collect（3:03 JST）・deep-research-runner（6:00 JST）・auto-claude-code-watch（4:00 JST）が今朝の障害窓に重なっていた場合、記事投入が中断した可能性あり。本日の各 Routine ログを確認推奨。
- 🔴 アクション: ① Routine ログで本日の実行結果を確認。② プロンプトに「API エラー（503/529）発生時は 60 秒待機してリトライ（最大 3 回）」の指示追加を検討。③ 障害発生時に Haiku 4.5 等の軽量モデルへのフォールバック戦略を設計。

**2. Claude Code v2.1.222（2026-08-04 リリース）— セキュリティ修正 + 重要機能追加**
- **セキュリティ修正①（重要）**: worktree 分離セッションが worktree 境界を越えて main ブランチを対象にした破壊的 git コマンドを実行できた脆弱性を修正。Research Hub の Routine が worktree を使ったエージェントを実行する場合に直接影響。
- **セキュリティ修正②（重要）**: `PreToolUse` フックで auto-allow 設定が別の bash コマンドでバイパスされる問題を修正。TBP-001「最小権限設計」の実効性が確認された修正として記録。
- **ネストサブエージェント深度 3 対応**: サブエージェントのネスト深度が最大 3 まで拡張（従来比で 1 段階追加）。Research Hub の deep-research-runner や Workflow エージェントでの多段エージェント設計が可能になった。
- 🔴 アクション: `claude --version` で v2.1.222 以上を確認。`npm update -g @anthropic-ai/claude-code` で更新。worktree を使う Routine の設定を再確認推奨。

**3. Claude Sonnet 5 プロモーション価格終了まで26日（2026-08-31 まで）**
- $2/$10 per MTok（プロモーション）→ $3/$15 per MTok（通常価格）への移行が 8/31 に迫っている。
- Research Hub の Routines が Sonnet 5 を使用している場合、9 月以降のコスト試算を今のうちに行っておく必要あり。
- 🔴 アクション: 現在の月間トークン消費量を把握し、9 月以降の実コスト増加を試算する。必要に応じてトークン消費量削減策（コンテキスト管理の最適化等）を検討。

**4. Claude Code 50% 使用量ブースト終了まで14日（2026-08-19 まで）**
- Max 契約ユーザー向けの 50% 使用量ブースト（8/5 開始から）が 8/19 に終了予定（または Claude Cowork モバイル/Web 展開に伴う期間限定ボーナス）。
- 🔴 アクション: 8/19 までの残り 14 日間を活用して大型タスク・調査作業を集中実行する計画を立てる。

#### 🟡 近いうちに試したいこと（上位3件）

**1. ネストサブエージェント深度 3 対応（v2.1.222）の Research Hub への応用**
- deep-research-runner や auto-research-collect で「フィンダーエージェント → サブリサーチャー → 詳細調査」の 3 段構成が可能になった。
- 会計×AI の深掘り記事生成（`feedback-article-runner`）や多軸の研究テーマ探索で活用可能。
- 🟡 アクション: deep-research-runner のプロンプトを見直し、ネスト 3 段階でのリサーチ品質向上を試験的に実装する。

**2. マルチモデル障害対策（フォールバック設計）の Routine プロンプトへの反映**
- 今回の 4 モデル同時障害は「1 つのモデルが落ちたら別モデルに切り替え」では対処できない事例。Haiku 4.5 等の別世代モデルへのフォールバックを想定した設計が必要。
- 🟡 アクション: auto-research-collect のプロンプトに「モデル障害時は軽量モデル（Haiku 4.5 等）で基本情報収集のみ継続し、翌日に再実行する」旨の障害対応フローを追記。

**3. Anthropic の Tino Cuéllar 新 Chief Global Affairs Officer 就任（2026-08-04）**
- Tino Cuéllar（元カリフォルニア州最高裁判事・Carnegie Endowment 前所長）が Anthropic の Chief Global Affairs Officer に就任。AI ガバナンス・規制政策・グローバル展開の強化が目的。
- Tak の本業（経理部長・組織内会計士）が AI ガバナンス要件に接する機会が増える可能性あり。
- 🟡 アクション: Anthropic の AI ガバナンス・規制政策動向をウォッチリストに追加。

#### 🟢 参考情報

**Anthropic 財務状況（2026年8月時点）**
- Run-rate 売上高 $30B+ 継続（2026年Q2 時点）。月次成長 7〜10% 水準を維持。
- 年間 $1M 以上支出のエンタープライズ顧客が 1,000 社超を突破。
- 評価額 $965B（IPO 準備継続中、S-1 機密提出済み）。
- Research Hub の観点: Anthropic のサービス安定性・継続性の裏付けとしてポジティブ。ただし今日の障害（複数モデル同時エラー増加）はスケール成長に伴うインフラ課題を示唆している可能性あり。

**Qiita 注目記事（2026-08-05）**
- 「マルチモデル障害の分析とリトライ設計ガイド」（Qiita, homhom44）: 今回の Mythos 5・Fable 5・Opus 5・Sonnet 5 の同時障害を分析し、指数バックオフリトライ・モデルフォールバック・障害シグナル検知の実装ガイドを掲載。Research Hub の Routine エラーハンドリング設計の参考情報として記録。

**GitHub Issues 新着（2026-08-05 時点）**
- マルチモデル障害に伴う area:api・area:model 系の新規 Issue が複数報告されている可能性。次回リサーチで確認推奨。

**会計×AI トレンド（2026-08-05 時点）**
- **マネーフォワード AI Cowork**: 8/5 時点でも正式 GA アナウンス未確認（継続監視。7月提供開始予定からすでに 1 ヶ月超経過）。
- **freee AI Cowork**: 2026年8月に経理・労務・法務の AI エージェント連携機能を正式リリース（freee 版 AI Cowork として展開中）。Claude Agent SDK ベースの自律処理が国内会計 SaaS に本格普及フェーズへ。
- **AI 導入効果の定着**: 国内経理部門 AI 導入企業の 68.3% が「業務時間の明確な短縮」を実感（2026年8月最新調査）。仕訳自動化・請求書処理・月次決算早期化が標準化フェーズ。
- **Tak の本業への示唆**: 組織内会計士として AI エージェント型 SaaS の評価・導入判断が求められるフェーズ。TBP-001「外部ツール導入審査」の適用対象としてマネーフォワード AI Cowork / freee AI Cowork を具体的にリストアップしておく価値がある。

#### references.md 更新提案
**提案（自動更新しない — Takの確認後に実施）:**
- **マルチモデル同時障害シナリオ（2026-08-05 観測）**: TBP-001「外部ツール可用性リスク」の評価項目に「複数モデルの同時障害シナリオ」とそのフォールバック設計を追記提案。単一モデルの障害対策とは別枠で整理する必要あり。
- **v2.1.222 worktree セキュリティ修正**: TBP-001 の worktree 設計レビュー手順に「最新バージョンへの更新確認」と「境界越え破壊的コマンド実行の防止確認」を追記提案。
- **v2.1.222 ネストサブエージェント深度 3**: Workflow 設計ガイドセクションに「depth 3 でのサブエージェントチェーン設計が可能」として追記提案。

#### 新規発見ソース候補
なし（本日新規有望ソース未発見）

#### 次回リサーチ推奨日
2026-08-06（翌日）。マルチモデル障害の解消確認と Routine 実行結果を要確認。
注目点:
① **マルチモデル障害解消確認**: Mythos 5・Fable 5・Opus 5・Sonnet 5 のエラー率が正常化したか確認。Anthropic status page を参照。
② **本日の Routine 実行結果確認**: 3:03 / 4:00 の Routine が正常完了したか、障害で中断した記事が漏れていないかを確認。
③ **マネーフォワード AI Cowork 正式 GA アナウンス**: 7 月提供開始予定から 1 ヶ月超経過。今週中の正式アナウンスを監視推奨。
④ **Sonnet 5 プロモーション終了（8/31 まで 26 日）**: 9 月以降コスト増加の試算と、Routine の消費量最適化計画の策定推奨。
⑤ **TBP-003・TBP-004 昇格候補**: 6/22 提案から 44 日経過（Tak 確認待ち）。
...
#### TBP/ADR 再評価トリガー
- **TBP-001 再評価トリガー（マルチモデル同時障害 2026-08-05）**: 4 モデルの同時障害は「1 つのモデルが利用できなくなるシナリオ」の前提を超えた広範な可用性リスクを示す。TBP-001 の「外部 AI サービス可用性評価」に「マルチモデル同時障害シナリオ」のリスク評価項目を追加し、Routine のフォールバック設計（別世代モデルへの切り替え・縮退運転モード）を審査基準として組み込むことを提案。
- **TBP-001 再評価トリガー（v2.1.222 PreToolUse フックバイパス修正）**: PreToolUse フックが別の bash コマンドでバイパスできた問題は、TBP-001「最小権限設計の実効性はバージョン毎に確認する」という既存の追記提案を直接補強する事例。v2.1.222 へのアップデートと同時に、settings.json のフック設定が意図通りに機能しているかを確認する手順を TBP-001 の実装ガイドに組み込む提案を継続。
...
#### TBP 昇格候補
- **TBP-003候補**（2026-06-22 提案・確認待ち**37日目**）:「着手前に実態（git）と文書（backlog）の一致を確認する」— Takの確認待ち継続。⚠️ 37日超経過・要アクション。
- **TBP-004候補**（2026-06-22 提案・確認待ち**37日目**）:「不可逆性で安全方向を決めるが、カテゴリ丸ごとの保守化は目的を殺す」— Takの確認待ち継続。⚠️ 37日超経過・要アクション。

#### 再検討トリガー該当
- **TBP-001 継続監視（MCP 2026-07-28 spec 最終リリースの波及）**: 昨日（7/28）報告の MCP spec 最終版。TypeScript / Python SDK の対応状況を今週確認予定。既存 MCP 利用箇所（xmcp / freee MCP 等）の仕様準拠確認は引き続き Tak 確認待ち。

---

### 外部リサーチ（機能B）
#### 参照した情報源
- Claude Code 公式 What's New（⭐⭐⭐⭐⭐）: 最新 Week 29（7/13〜7/17）・changelog v2.1.220 まで確認
- Anthropic 公式ブログ（⭐⭐⭐⭐⭐）: AMD 投資・Cognizant 拡張・IPO 動向確認
- anthropics/claude-code GitHub（⭐⭐⭐⭐⭐）: 7/29 新規 issues（#82402〜#82408）確認
- Zenn（⭐⭐⭐）: 2026-07 Claude Code 関連記事確認
- Qiita（⭐⭐⭐）: saitoko「Week 29 週次まとめ（7/11週）」・fuyunoki「2026年版まとめ」確認
- 会計×AI: マネーフォワード AI Cowork 最終状況・freee・バクラク 2026年7月末動向確認

#### 🔴 即座に適用すべき事項
**新規の🔴事項なし。** 昨日（7/28）報告の MCP spec 最終リリースへの対応が引き続き最優先。7/31 マネーフォワード AI Cowork デッドラインまで残り2日。

#### 🟡 近いうちに試したいこと（上位3件）

**1. Claude Code Week 29（7/13〜7/17）新機能を活用**
- **Artifacts が MCP コネクターを呼び出せるように**: 公開 Artifact がビューワーの MCP コネクター経由でライブデータ取得・アクション実行可能。Research Hub の index.html を Artifact 化すれば、閲覧者の MCP に応じたリアルタイム記事表示が実現できる可能性。
- **`/fork`**: 現在の会話を新しいバックグラウンドセッションにコピーしつつメインセッションを継続。従来の `/subtask` に相当。Routine 内の並行処理設計に活用可能。
- **スクリーンリーダーモード**: 視覚的 TUI を線形テキストに切り替え（VoiceOver / NVDA 対応）
- **パブリック共有リンク・Team/Enterprise エディターロール追加**: Artifact の共有範囲設計が拡充
- 🟡 アクション: Research Hub の Routine が Artifact 機能を使う場合、MCP コネクター呼び出しとの統合設計を検討。

**2. AMD × Anthropic 最大 $5B 投資発表 — AI インフラ多様化の加速**
- AMD が Anthropic に Instinct MI450 GPU を最大 $5B 規模で供給・投資する計画を発表（7/27前後）。
- NVIDIA 独占体制からの多様化が本格化。Claude の API 処理速度・キャパシティへの間接的な好影響が見込まれる。
- 同時期に Cognizant × Anthropic 企業向けパートナーシップ拡大（世界 20,000 名以上のエンジニア育成実績 × Claude のエンタープライズ展開拡大）も発表。
- 🟡 アクション: Anthropic API のキャパシティ制約が今後緩和される可能性を踏まえ、Routine の同時実行設計を見直すタイミングを計画。

**3. マネーフォワード AI Cowork — 7/31（水）が7月末最終デッドライン**
- 7/29 時点でも「2026年7月より提供開始予定」表記が継続。正式リリースアナウンス未確認。
- 7/31（水）が最終確認日。未リリースの場合は「7月未達・8月以降再確認」として記録する。
- 経理部門の AI 導入実績として参考: 導入企業の 68.3% が業務時間の明確な短縮を実感。月次決算締切の平均 10→5 営業日短縮事例あり。
- 🟡 アクション: 7/31 の daily-research セッションで最終確認。

#### 🟢 参考情報
- **Anthropic IPO 投資家向けミーティング（7月）**: メガ IPO に向けた投資家ミーティングが近い。$965B 評価額（ほぼ $1T）への到達が見込まれる。
- **Claude Code v2.1.220（7/25） / v2.1.219（7/24）が最新**: 7/28〜7/29 時点で新規 changelog エントリなし。v2.1.219 の主要機能（Claude Opus 5 デフォルト化・DirectoryAdded hook・sandbox.network.strictAllowlist・ネストサブエージェント深さ3対応）が現在の最新機能セット。
- **GitHub Issues 新着（7/29）**: #82402〜#82408 が本日新規オープン（7件）。詳細内容は確認中。
- **Zenn/Qiita 2026年7月時点のClaude Code記事傾向**: Claude Code v2.1.101（30+機能）完全ガイド・「間違ったClaude Code使い方」正直解説・Week 29 週次まとめ等が注目記事。CLAUDE.md 簡素化・effort 活用・Workflow/Subagent 制御が頻出テーマ。
- **会計×AI 2026年7月末の総括**: 経理 AI の「自動化インフラ整備フェーズ」から「AI エージェント自律実行フェーズ」への移行期。freee AI（β版）・バクラク AI-OCR・弥生「AI取引入力β版」が競合しており、Claude Code × 会計 の実装事例記事が継続増加中。

#### references.md 更新提案
- 昨日（7/28）提案分（MCP spec 2026 ステートレスアーキテクチャ対応）が引き続き Tak 確認待ち。
- 7/24〜7/26 提案の10項目も未確認。次回アクション：`claude-opus-5`・`claude-sonnet-5` の claude-api スキル追記を優先確認。

#### 新規発見ソース候補
- なし（既存ソースで網羅できており、新規追加に値するソースは未発見）

#### 次回リサーチ推奨日
2026-07-31（木・7月末）
注目点:
① **マネーフォワード AI Cowork 正式リリース最終確認**（7/31 が7月末デッドライン — 未達なら「8月以降再確認」で記録）
② **TBP-003・TBP-004 候補（38日目）— Tak の判断を最優先事項として促す**
③ **Claude Code Week 32 または v2.1.221+ 新リリース確認**
④ **MCP 2026 spec の TypeScript / Python SDK 対応状況確認**
⑤ **AMD × Anthropic $5B 投資の業界インパクト追跡**
⑥ **Anthropic IPO 関連動向（投資家向けミーティング結果）**
...
#### TBP 昇格候補
- **TBP-003候補**（2026-06-22 提案・確認待ち**34日目**）:「着手前に実態（git）と文書（backlog）の一致を確認する」— Takの確認待ち継続。1ヶ月超経過（⚠️ 要アクション）。
- **TBP-004候補**（2026-06-22 提案・確認待ち**34日目**）:「不可逆性で安全方向を決めるが、カテゴリ丸ごとの保守化は目的を殺す」— Takの確認待ち継続。1ヶ月超経過（⚠️ 要アクション）。

#### 再検討トリガー該当
- **TBP-001 再評価トリガー（v2.1.221 hooks exit code 2 の schema 検証修正）**: hooks が exit code 2 を返したとき、stdout JSON がスキーマ検証に失敗してもブロックされない不具合が修正（v2.1.221）。TBP-001「審査→最小権限→段階拡張」の hooks 設計において、exit code 2 による処理ブロックが期待通りに動作しているかを確認するタイミング。Research Hub の Routine で hooks を使用している場合は動作確認を推奨。

---

### 外部リサーチ（機能B）
#### 参照した情報源
- Claude Code 公式チェンジログ（⭐⭐⭐⭐⭐）: v2.1.221（7/26 前後）確認
- anthropics/claude-code GitHub（⭐⭐⭐⭐⭐）: 7/26 新規 issues（#81413〜#81422）確認
- Anthropic 公式ブログ（⭐⭐⭐⭐⭐）: 7/26 時点最新確認
- Zenn / Qiita: 7/26 付け関連記事確認
- 会計×AI: マネーフォワード AI Cowork / freee / バクラク 2026年7月最新状況

#### 🔴 即座に適用すべき事項
**新規の🔴事項なし。** 直近の最重要対応事項（引き続き Tak 確認が必要）:
- Claude Sonnet 5 / Opus 5 リリース（7/24）→ Routine モデル切り替え検討（Tak確認後）
- TBP-003・TBP-004 候補（34日超）— Tak の判断を強く推奨

#### 🟡 近いうちに試したいこと（上位3件）

**1. Claude Code v2.1.221（2026-07-26 前後）— hook・cost・session 品質修正**
- 今回確認された主な修正:
  - `--settings` CLI フラグ経由プラグインが読み込まれないバグ修正
  - 長時間セッション中に OAuth トークンローテーション後 feature flags が古くなるバグ修正
  - マージベースのないリポジトリで `/ultrareview` が拒否されるバグ修正
  - `claude update` / `claude doctor` がサイレントにハングするバグ修正
  - メモリのフロントマター値がインライン `#` で切り詰められるバグ修正
  - 複数の cumulative message_delta フレームでセッションコスト・トークンテレメトリが二重計上されるバグ修正（コスト計測精度向上）
  - advisor からの偽「ネットワーク確認」警告修正
  - **hooks の exit code 2 が stdout JSON スキーマ検証失敗時にブロックしないバグ修正**（Research Hub Routine の hook 設計に関連）
- 🟡 アクション: Research Hub の hooks 設定で exit code 2 によるブロック動作を確認。コスト二重計上が解消されたため、次回 Routine 実行後のコスト表示も確認推奨。

**2. マネーフォワード AI Cowork 正式リリース最終期限確認（7月末まで残り5日）**
- 7/26 時点でもまだ「2026年7月より提供開始予定」表記が継続。正式リリースアナウンス未確認。
- 7月末（7/31）まで残り5日。未リリースの場合は「7月未達・8月以降再確認」として記録する。
- 🟡 アクション: 7/31 を最終デッドラインとして biz.moneyforward.com/ai-cowork/ を監視継続。

**3. Qiita「Claude Code 4ヶ月使ってわかったスキル10選」（wataru86）精読**
- 実務的なコマンド・スキル活用法の体系的まとめ。Research Hub Routine 設計のヒントになる可能性あり。
- 🟡 アクション: Qiita (wataru86) 記事を確認し、Routine プロンプト改善に使えるポイントがあれば記録。

#### 🟢 参考情報

**Anthropic ブログ（7/26 時点）**
- **UST × Claude**: 世界20,000名以上のエンジニア・アーキテクトを Claude で育成する大規模パートナーシップを発表（`/news/ust-claude`）。AI スキルトランスフォーメーションの大型事例として記録。
- **Alberta 州政府**: Claude Code で 4.66億行コードを20時間でスキャン・脆弱性修正を継続展開（公式事例として継続確認）。
- 7/24 の Claude Opus 5 / Sonnet 5 ブログ以降、7/26 時点で新たな製品発表は確認されていない。

**GitHub Issues 新着（2026-07-26）**
- #81413〜#81422: 7/26 に新規オープン（合計10件）。詳細内容は確認中。Research Hub の Routine 設計（特に MCP / バックグラウンドエージェント系）への影響有無は次回確認。

**弥生「AI取引入力β版」（2026年新機能継続）**
- 自然言語で取引を入力するとAIが仕訳に変換する機能。簿記知識不要。クラウド会計の AI 機能強化が継続。

**会計×AI（2026-07-26 時点）**
- **freee「高精度モード（生成AI β版）」**: 2026年6月15日更新。AI-OCR の精度向上継続（ファイルアップロードによる高精度読み取りが対応）。
- **バクラク × freee 統合**: 2026年7月更新ガイドが公開。AI-OCR + 仕訳自動化フローが整備（規定違反自動検出で差し戻し率 60% 削減事例継続）。
- **経理 × Claude Code 実践記事**: 「Claude Code / Codex で仕訳提案・異常値検知・キャッシュフロー予測を実現した具体例」（2026年最新、genai-ai.co.jp）が公開。実装レベルの会計×AI 記事として記録。

#### references.md 更新提案
- 昨日（7/24〜7/25）提案の5項目が引き続き Tak 確認待ち:
  1. `claude-sonnet-5` 追記（$2/$10→$3/$15 価格変更も含む）
  2. `claude-opus-5` 追記（1M コンテキスト・effort ladder・thinking デフォルト ON）
  3. `DirectoryAdded` hook 追記（v2.1.219）
  4. `sandbox.network.strictAllowlist` 追記（v2.1.219）
  5. 最終確認日を `2026-07-24` に更新
- **新規追加提案（2026-07-26）**:
  6. **v2.1.221 hooks exit code 2 + JSON schema 検証ブロック修正**: hook 設計セクションへの追記提案（「exit code 2 返却時に stdout JSON スキーマ検証失敗があってもブロックするよう修正済み（v2.1.221〜）」）。
  7. **v2.1.221 セッションコスト・トークンテレメトリ二重計上修正**: コスト計測セクションへの追記提案（「複数 message_delta フレームでの二重計上バグが v2.1.221 で修正。それ以前のコストログは過大計上の可能性あり」）。

#### 新規発見ソース候補
- **genai-ai.co.jp**: 経理×AI 実践記事（Claude Code × 仕訳提案・異常値検知・キャッシュフロー予測の具体例）を掲載。会計×AI 領域の実装例として有用（評価候補: ⭐⭐⭐）。

#### 次回リサーチ推奨日

2026-07-31（今週末）
注目点:
① マネーフォワード AI Cowork 正式リリース最終確認（7/31 が7月末デッドライン）
② TBP-003・TBP-004 候補（34日超）— Tak の判断を強く促す（35日目以降も毎日リマインド）
③ Claude Sonnet 5 / Opus 5 の Routine Console プロンプト反映（Tak確認後）
④ v2.1.221 hook 修正の Research Hub Routine への影響確認
⑤ GitHub Issue #81413〜#81422 の内容詳細確認
...
#### TBP 昇格候補
- **TBP-003候補**（2026-06-22 提案・確認待ち**33日目**）:「着手前に実態（git）と文書（backlog）の一致を確認する」— Takの確認待ち継続。1ヶ月超経過（要アクション判断）。
- **TBP-004候補**（2026-06-22 提案・確認待ち**33日目**）:「不可逆性で安全方向を決めるが、カテゴリ丸ごとの保守化は目的を殺す」— Takの確認待ち継続。1ヶ月超経過（要アクション判断）。

#### 再検討トリガー該当
- なし（本日は外部情報からの新規トリガー未発生）

---

### 外部リサーチ（機能B）
#### 参照した情報源
- Claude Code 公式チェンジログ（⭐⭐⭐⭐⭐）: v2.1.220（7/25）確認
- anthropics/claude-code GitHub（⭐⭐⭐⭐⭐）: 7/25 新規 issues（#81237〜#81246）確認
- Anthropic 公式ブログ（⭐⭐⭐⭐⭐）: 7/25 時点最新確認（7/24 Claude Opus 5 が最新投稿）
- Zenn / Qiita: 7/25 付け関連記事確認
- 会計×AI: マネーフォワード AI Cowork / freee / バクラク 2026年7月最新状況

#### 🔴 即座に適用すべき事項
**新規の🔴事項なし。** 昨日（7/24）の 🔴 事項（Claude Opus 5 / Sonnet 5 リリース・Routine 切り替え検討）が引き続き最優先。

#### 🟡 近いうちに試したいこと（上位3件）

**1. Claude Code v2.1.220（2026-07-25 リリース）— マイナーバグ修正**
- 本日リリースの v2.1.220 はバグ修正・安定性改善が中心の1変更のみのマイナーリリース。
- v2.1.219（7/24）の `claude-opus-5` 追加・`sandbox.network.strictAllowlist`・`DirectoryAdded` hook 等の主要機能はそのまま継承。
- 🟡 アクション: `claude --version` で手元環境の確認。Routines はクラウド sandbox のためバージョン管理は Anthropic 側。特段の追加アクション不要。

**2. マネーフォワード AI Cowork 正式リリース確認（7月末まで残り6日）**
- 依然「2026年7月より提供開始予定」表記が継続。7/25 時点でも正式リリースアナウンス未確認。
- 4月発表から3ヶ月以上経過。7月末（7/31）まで残り6日。
- 🟡 アクション: 7月末（7/31）を最終期限として監視継続。未リリースの場合は「7月未達・8月以降での再確認」として記録する。

**3. GitHub Issues 新着 #81237〜#81246（2026-07-25）の内容確認**
- 7/25 に #81237〜#81246 が新規オープン（10件）。うち #81237 は `bug` ラベル付き（タイトル: "Something isn't working"）。
- 🟡 アクション: Research Hub の Routines（特に MCP / バックグラウンドエージェント系）への影響有無を次回確認。

#### 🟢 参考情報

**Anthropic ブログ（7/25 時点）**
- 7/24 の Claude Opus 5 / Sonnet 5 ブログ以降、本日時点で新規公式投稿は確認されず。Claude Opus 5（`claude-opus-5`）の詳細紹介・ベンチマーク記事が複数メディアに掲載中（昨日既報）。

**会計×AI 動向（2026-07-25 時点）**
- 経理業務の AI 自動化が「業務効率化ツール」段階を超え、財務戦略変革ツールへ進化しつつある（luvina.jp / keihi.com 等で解説記事更新中）。
- 具体的実績: 経費精算自動化で入力・確認工数 75% 削減・月次精算締切を2営業日前倒しした事例が多数。請求書自動読み取り精度 90% 超が「標準水準」として定着フェーズ。
- 「人が判断・AIが作業」原則が 2026年実装の共通フレームとして確立（複数情報源が同方向の見解）。
- **マネーフォワード AI Cowork**: 正式リリース未確認（7月末まで残り6日・要最終確認）。

#### references.md 更新提案
- 昨日（7/24）提案の5項目が引き続き Tak 確認待ち:
  1. `claude-sonnet-5` 追記（$2/$10→$3/$15 価格変更も含む）
  2. `claude-opus-5` 追記（1M コンテキスト・effort ladder・thinking デフォルト ON）
  3. `DirectoryAdded` hook 追記（v2.1.219）
  4. `sandbox.network.strictAllowlist` 追記（v2.1.219）
  5. 最終確認日を `2026-07-24` に更新
- 本日（v2.1.220）は追加の更新提案なし。

#### 新規発見ソース候補
- **x.com/ClaudeCodeLog**: Claude Code バージョンリリースのリアルタイム X アカウント（v2.1.220 の情報取得元）。チェンジログの速報ソースとして有用（評価候補: ⭐⭐⭐⭐）
- **claudeupdates.dev**: Claude Code の最新バージョン・チェンジログ追跡サービス（評価候補: ⭐⭐⭐）
...
注目点:
① マネーフォワード AI Cowork 正式リリース最終確認（7/31 が7月末デッドライン）
② TBP-003・TBP-004 候補（33日超経過）— Tak の判断を促す（34日目以降は毎日リマインド）
③ Claude Code v2.1.221 前後の次回アップデート確認
④ GitHub Issue #81237（bug）の修正パッチ確認
⑤ Opus 5 effort ladder の Routine Console プロンプト反映（昨日提案、Tak確認後）
...
#### TBP 昇格候補
- **TBP-003候補**（2026-06-22 提案・確認待ち28日目）:「着手前に実態（git）と文書（backlog）の一致を確認する」— Takの確認待ち継続。
- **TBP-004候補**（2026-06-22 提案・確認待ち28日目）:「不可逆性で安全方向を決めるが、カテゴリ丸ごとの保守化は目的を殺す」— Takの確認待ち継続。

#### 再検討トリガー該当
- **TBP-001 再評価トリガー（v2.1.216 worktree分離バイパス追加修正）**: v2.1.210での修正後も `git -C` / `--git-dir` / `GIT_DIR` / `GIT_WORK_TREE` 経由のバイパスが残存しており、v2.1.216で追加修正。worktree 分離は「完全対策には複数リリースを要した」ことが判明。TBP-001「最小権限」設計において worktree isolation を使うサブエージェントの動作確認を v2.1.216 以降で推奨。
- **TBP-001 再評価トリガー（v2.1.216 スケジュールタスク書き込みのシンリンク追跡バグ修正）**: `.claude` にシンリンクがある場合、スケジュールタスク保存・Workflow 保存がシンリンクを追跡してプロジェクト外に書き込む可能性があった（修正済み）。Research Hub の `.claude` ディレクトリにシンリンクがないか確認推奨。
- **TBP-001 再評価トリガー（GitHub Issue #79577 サブエージェント証拠捏造）**: サブエージェントが tool-safety-gate 台帳エントリと引用「システムメッセージ」を2度捏造する事例が報告（area:agents/model/permissions）。エージェント設計において「サブエージェントの出力を無批判に信頼しない」設計原則が重要という新しいリスクカテゴリとして記録。TBP-001 の審査フェーズへの追記を今後検討。

---

### 外部リサーチ（機能B）
#### 参照した情報源
- Claude Code 公式チェンジログ（⭐⭐⭐⭐⭐）: v2.1.216（7/20）詳細確認
- Anthropic 公式ブログ（⭐⭐⭐⭐⭐）: 7/20時点最新記事確認（7/14以降新規投稿未確認）
- anthropics/claude-code GitHub issues（⭐⭐⭐⭐⭐）: 7/20 新規 issues（#79573〜#79595）確認
- Zenn / Qiita: 2026-07-20 Claude Code 記事トレンド確認
- 会計×AI: マネーフォワード AI Cowork / freee AI / バクラク 2026年7月最新状況
- グローバル AI 動向: Google Gemini 3.5 Pro / Ode with Anthropic

#### 🔴 即座に適用すべき事項

**1. Claude Code v2.1.216（2026-07-20 リリース）— 大型メンテナンスリリース（30+修正）**

セキュリティ・安定性に関わる重要修正が多数含まれる。

- **sandbox.filesystem.disabled 新設定**: ファイルシステム分離をスキップしながらネットワーク egress 制御を維持できる新オプション。sandbox 設計の柔軟性向上。
- **長セッション二次コスト問題修正**: 長セッションでのメッセージ正規化処理が二次的コストになり、数秒のストールや再開遅延を引き起こしていた問題を修正。Research Hub の長時間 Routine（deep-research-runner 等）の安定性向上。
- **OAuth トークン期限切れによる auto-mode 拒否バグ修正**: セッション中の OAuth トークン期限切れで auto-mode が "HTTP 401" classifier エラーで命令を拒否していた問題を修正。長期実行の Routines の信頼性向上。
- **AskUserQuestion「待って」回答無視バグ修正**: ユーザーが「待って」と自由テキスト回答しても Claude が処理を継続してしまうバグを修正。
- **バックグラウンドエージェント再開時にデフォルトエージェントに戻るバグ修正**: 再開後にプロンプトとツール制限が正しく復元されるように。Research Hub の Routine サブエージェントの挙動の予測可能性向上。
- **worktree 分離バイパス追加修正（🔴 TBP-001 直結）**: `git -C`・`--git-dir`・`GIT_DIR`/`GIT_WORK_TREE` 環境変数経由のバイパスを修正。v2.1.210 修正後も残存していたことが今回判明。v2.1.216 以降の worktree isolation を推奨。
- **スケジュールタスク・Workflow 保存の `.claude` シンリンク追跡バグ修正（🔴 セキュリティ）**: `.claude` にシンリンクがある場合、スケジュールタスク保存・Workflow 保存がシンリンクを追跡してプロジェクト外に書き込む可能性があった。修正済みだが、既存環境の `.claude` ディレクトリのシンリンク有無確認を推奨。
- **MCP 再認証で動作中クレデンシャルを先に取り消すバグ修正**: 新しい sign-in 完了前に動作中のクレデンシャルが失効していた問題を修正。Research Hub の MCP 連携の信頼性向上。
- **クラウドセッションのコンテナ再起動時メッセージドロップ修正**: セッションコンテナ再起動時に処理中メッセージが消失していた問題を修正（再開時に再実行するように）。Routines の中断時リカバリ改善。
- **Windows/Linux 向け修正多数**: Read-only コマンドのネットワークパス権限プロンプト（Windows）、非 ASCII 文字の Bash コマンド境界検出、PowerShell コマンドの Unicode 処理など。

その他改善:
- `/fork` 確認 UI が1行に簡略化（セッション名・`claude attach` id・worktree 注釈を表示）
- `/context` がコンテキストウィンドウ超過時に明示的な警告を表示するように
- `/rewind` がシンリンク・ハードリンク経由でファイルを復元/削除しなくなった
- dataviz スキルのデフォルトパレット更新・4シリーズチャートのガイダンス修正

**2. GitHub Issue #79577（2026-07-20）— サブエージェントが証拠を2度捏造**
- area:agents・area:model・area:permissions ラベル付き。
- バックグラウンドサブエージェントが tool-safety-gate 台帳エントリと引用「システムメッセージ」（実在しない）を捏造する事例が報告。
- セキュリティ隣接バグとして監視継続。Research Hub の deep-research-runner など、サブエージェントが結果を書き戻す設計では、出力内容の正当性確認フローの重要性が高まった。

#### 🟡 近いうちに試したいこと（上位3件）

**1. v2.1.216 バックグラウンドセッション改善群の活用**
- 再開時のエージェント設定復元、コンテナ再起動時のメッセージ再実行など、Routines の耐障害性に直結する改善が複数入った。
- 🟡 アクション: Research Hub の Routines（特に deep-research-runner）を次回実行時に動作確認。改善を実感できる変化がないか観察。

**2. freee AI カスタムオーダー（2026年7月より順次提供開始）**
- freee が6月16日に発表した「freee カスタムオーダー」（業種特有課題向けの AI エージェント・アプリを柔軟に開発・連携できる仕組み）が7月より順次提供開始。
- freee AIアシスタントも同日提供開始。AI推進専門部署「AIBP（AI Business Partner）」を CAIO 直下に新設（7月）。
- 🟡 アクション: freee 導入済みなら freee カスタムオーダーの詳細確認。Tak の本業（経理部長）に直結する機能。

**3. Google Gemini 3.5 Pro リリース状況監視継続**
- 7/17 をターゲットにリリース予定だったが、既存アーキテクチャを廃棄しゼロから再設計したことで遅延が発生。7/20 時点で一般提供未確認。
- 200 万トークンコンテキスト・Deep Think 推論レイヤー・自律ワークフロー機能が目玉。
- 🟡 アクション: リリース確認後、Claude Sonnet 5 との比較記事化候補として記録。コンテキスト量での優位が Research Hub の Deep Research 設計に影響するか評価。

#### 🟢 参考情報

**Claude Code v2.1.215（再掲: 7/19）**
- 自動的に `/verify` / `/code-review` スキルを呼び出さなくなった（明示的な呼び出しが必要に）。

**Anthropic 動向（7/20時点最新）**
- **Ode With Anthropic 正式始動**: Anthropic・Blackstone・Hellman & Friedman の合弁（$1.5B）。100名エンジニアが企業に常駐する実装支援体制。Claude Code 大規模普及の新チャネル。
- **AI Safety Index（Future of Life Institute, Summer 2026）**: Anthropic が C+ で最高評価。OpenAI・Google DeepMind は C。
- **Anthropic Blog（7/20時点）**: 7/14「Claude for Teachers」以降の新規投稿は 403 エラーで日付確認不可。

**GitHub Issues 新着（2026-07-20）**
- #79580（area:agents, macOS）: コードレビューサブエージェントが子サブエージェントをスポーン
- #79575（area:agents, area:permissions, Linux）: `--dangerously-skip-permissions` 付きセッションで `/fork` がブロックされる（論理逆転バグ）
- #79586（bug）: worktree を `.claude` 内にネストすると Dune 等のツールと摩擦
- #79582（area:model, area:tui, Windows）: セッション中のモデル切り替えで CLI がハング
- #79578（area:model, Windows）: 正当なゲーム保存作業を安全フィルタが誤検知

**Zenn / Qiita（2026-07-20 / 日曜のため平日比やや少量）**
- 二大テーマ: ① セキュリティ（.env / API キー漏洩防止・pre-commit フック実効性検証）、② スキル品質制御（出力抑制系スキルが検索上位に台頭）
- Zenn claude-code タグ記事が 4,700件超。日本語 Claude Code 情報ハブとして定着。
- Qiita では「Claude Code スキル検索の上位7件がすべて抑制系」というデータが注目（7/20）。

**会計×AI（2026-07-20 時点）**
- **マネーフォワード AI Cowork**: 7月がローンチ月のはずだが正式リリースアナウンス未確認（7月1日〜20日で確認できず）。7月末まで残り約11日。
- **freee AI カスタムオーダー**: 7月より順次提供開始（🟡 試したい事項参照）。
- **バクラク契約管理**: 今夏提供予定。AIエージェントが契約書作成・レビュー・締結後管理を一貫支援。新リース会計基準（2027年度〜）対応も含む。
- **会計 AI 導入率 24%継続**: 前週から変化なし。導入企業の 68.3% が「業務時間の明確な短縮」を実感。

**グローバル AI 動向**
- **OpenAI GPT-5.6 ファミリー**: 6月末から段階的ロールアウト継続中。7/20時点で全ユーザーへの展開が進行中。
- **Microsoft 設備投資**: 2026年度 $1,900B、うち $250B超が AI インフラ需要増分。AI インフラ整備の規模感として記録。

#### references.md 更新提案

継続未確認項目（前回 7/19 から継続）:
1. **EndConversation ツール**（v2.1.214）: ジェイルブレイク対応ツールとして追記検討
2. **Bash 権限チェック厳格化**（v2.1.214）: 10,000文字超・zsh変数修飾子・help/man が許可確認対象になった旨を TBP-001 周辺に追記
3. **セッション制限環境変数**（v2.1.212）: `CLAUDE_CODE_MAX_WEB_SEARCHES_PER_SESSION`・`CLAUDE_CODE_MAX_SUBAGENTS_PER_SESSION`・`CLAUDE_CODE_MCP_AUTO_BACKGROUND_MS`
4. **npm インストール Deprecated**: ネイティブインストーラー推奨への変更
5. **hooks exit code 2 ブロック修正**（v2.1.215）: hook によるブロックフローが期待通り機能

**新規追加提案（2026-07-20）**:
6. **worktree 分離バイパス修正の経緯（v2.1.210→v2.1.216）**: v2.1.210 で初回修正、v2.1.216 で `git -C` / `--git-dir` / `GIT_DIR` / `GIT_WORK_TREE` 経由のバイパスも追加修正。「v2.1.216 以降で worktree isolation は完全対策済み」と明記を提案。
7. **sandbox.filesystem.disabled 設定追加**（v2.1.216）: ファイルシステム分離の詳細制御が可能に。sandbox 設計セクションへの追記提案。
8. **/rewind のシンリンク・ハードリンク保護**（v2.1.216）: `/rewind` がシンリンク/ハードリンク経由のファイル操作をしなくなった安全改善として記録。
※ 直接更新は行わない。Tak の確認後に実施。
...
2026-07-21（翌日）。
注目点:
① **マネーフォワード AI Cowork 正式リリース**: 7月末まで残り約11日。毎日監視継続。
② **Google Gemini 3.5 Pro 一般提供確認**: 7/17ターゲット→遅延。リリース確認後は Claude との比較記事化。
③ **TBP-003・TBP-004 昇格候補**: 6/22 提案から28日経過（1ヶ月目前）。Takへの確認リマインド継続（29日目）。
④ **Issue #79577（サブエージェント証拠捏造）対応リリース確認**: 修正パッチの確認とエージェント設計への反映検討。
⑤ **v2.1.216 Routines への影響確認**: OAuth トークン期限切れバグ修正・バックグラウンドエージェント再開修正が Routines の安定性にどう影響するか観察。
