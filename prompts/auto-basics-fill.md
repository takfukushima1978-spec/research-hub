...
- **body_text 1500字必達は「見た目」でなく「HTMLタグ・空白を除いた実テキスト」で測られる**: 見た目で1900字書いても実テキストは約1300字に縮む（タグと空白が約3割を占める）。実運用の経験則では **見た目で2300字以上（地の文中心）書いてやっと実1500を超える**。**箇条書きは字数が稼げない**ので具体例の地の文を厚くする。投入前に `body_html` からタグ・空白を除いた長さを数え、1500未満なら拡充してから insert（無駄な400往復を防ぐ）
